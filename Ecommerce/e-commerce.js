...
function allProducts() {
  let mappedData = data.products
    .map((randomArr) => {
      // console.log(randomArr.);
      return ` <div class="product">
          <img src="${randomArr.images[0]}" alt="product" />
          <div class="product-header">
            <h2>${randomArr.title}</h2>
            <div class="price">$${randomArr.price}</div>
          </div>
          <h3>${randomArr.tags}</h3>
          <p class="description">
            ${randomArr.description}
          </p>
...
function getFragrances() {
  let filteredData = data.products.filter(
    (randomArr) => randomArr.category === "fragrances"
  );
...
function getBeauty() {
  let filteredData = data.products.filter(
    (randomArr) => randomArr.category === "beauty"
  );
...
function getfurniture() {
  let filteredData = data.products.filter(
    (randomArr) => randomArr.category === "furniture"
  );
...
function getGroceries() {
  let filteredData = data.products.filter(
    (randomArr) => randomArr.category === "groceries"
  );
...
function displayProducts(filteredProducts) {
  let mappedData = filteredProducts
    .map((product) => {
      return `
        <div class="product">
...
// Functions for specific categories
function getFragrances() {
  displayProducts(filterProducts("fragrances"));
}

function getBeauty() {
  displayProducts(filterProducts("beauty"));
}

function getFurniture() {
  displayProducts(filterProducts("furniture"));
}

function getGroceries() {
  displayProducts(filterProducts("groceries"));
}
